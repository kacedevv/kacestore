...
const SupportSection: React.FC<Props> = ({ initialTab = 'main' }) => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'faq':
        return (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 reveal">
            ...
            <div className="space-y-4 md:space-y-6">
              {FAQS.map((faq, i) => (
                <div key={i} className="bg-[#0b0f19] border border-white/5 p-6 md:p-10 rounded-3xl md:rounded-[3rem] hover:border-blue-500/30 transition-all group">
      ...
      case 'warranty':
        return (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 reveal">
            ...
            <div className="mt-10 md:mt-16 p-8 md:p-12 bg-blue-600/5 border border-blue-500/20 rounded-[2.5rem] md:rounded-[4rem] text-center relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600/10 blur-[100px]"></div>
                <p className="text-white font-black text-xl md:text-2xl uppercase tracking-tighter mb-8 md:mb-10 relative z-10">KACE STORE - AN TÂM SỬ DỤNG, ĐỒNG HÀNH BỀN VỮNG</p>
                <div className="flex flex-col sm:flex-row justify-center gap-4 md:gap-6 relative z-10">
                    <a href="https://zalo.me/0916882278" target="_blank" className="bg-blue-600 text-white px-10 md:px-16 py-4 md:py-6 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-blue-500 shadow-2xl transition-all">Liên hệ bảo hành ngay</a>
                </div>
            </div>
          </div>
        );
      ...
      case 'community':
        return (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 reveal">
            ...
            <p className="text-slate-400 text-lg md:text-2xl font-medium mb-12 md:mb-20 max-w-3xl leading-relaxed">Tham gia cùng cộng đồng hơn 50,000 khách hàng thân thiết để nhận các mẹo sử dụng AI, tài khoản free và thông tin ưu đãi sớm nhất.</p>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
              <div className="relative group overflow-hidden rounded-[2.5rem] md:rounded-[4rem] h-[350px] md:h-[450px] shadow-2xl">
        ...
        return (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 reveal">
            <div>
              ...
              <button onClick={() => setActiveTab('faq')} className="mt-8 md:mt-10 text-[10px] font-black text-blue-500 uppercase tracking-widest hover:text-white transition-colors">Xem tất cả câu hỏi</button>
            </div>
            <div className="flex flex-col justify-center">
              <div className="bg-[#0b0f19] border border-white/5 rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-16 relative overflow-hidden group shadow-2xl">
                ...
                <p className="text-slate-400 text-lg md:text-xl mb-10 md:mb-12 relative z-10 leading-relaxed font-medium">Đội ngũ kỹ thuật viên của Kace Store luôn sẵn sàng đồng hành cùng bạn để tối ưu hóa trải nghiệm số.</p>
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6 relative z-10">
                  ...
                  <button onClick={() => setActiveTab('community')} className="bg-white/5 border border-white/10 text-white px-10 md:px-12 py-4 md:py-6 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-white/10 transition-all">Gia nhập cộng đồng</button>
                </div>
                <div className="mt-10 md:mt-12 flex items-center gap-4 text-slate-600 relative z-10">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest">Support Online Now</span>
                </div>
              </div>
            </div>
          </div>
